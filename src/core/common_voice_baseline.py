...
def run_baseline(
    ...
    for i, ref in enumerate(tqdm(to_clone, desc="Cloning")):
        audio = impersonator.generate_impersonation(
            target_voice_path=str(ref),
            text=prompt_text,
            strategy="direct_cloning",
        )
        out_path = clone_out_dir / f"clone_{i:04d}.wav"
        impersonator.save_audio(audio, str(out_path), sample_rate=sample_rate)
        clone_paths.append(out_path)

    # Optionally downsample evaluation set to reduce scoring time
    eval_test_paths = split.test_paths
    if 0.0 < eval_downsample_frac < 1.0 and len(eval_test_paths) > 1:
